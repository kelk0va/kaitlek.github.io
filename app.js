...
class CompletionTracker {
    ...
    calculateCompletion() {
        ...
        items.push({
            name: `Tools Unlocked`,
            value: unlockedTools,
            status: unlockedTools >= 57 ? 'completed' : (unlockedTools > 0 ? 'partial' : 'missing'),
            description: `${unlockedTools}/57 tools unlocked${unlockedTools < 57 ? ` (${57 - unlockedTools} missing)` : ''}`
        });
        ...
        items.push({
            ...
            value: crestValue,
            status: crestValue >= 8 ? 'completed' : (crestValue > 0 ? 'partial' : 'missing'),
            description: `${crestValue}/8 crests unlocked${crestValue < 8 ? ` (${8 - crestValue} missing)` : ''}`
        });
        ...
        items.push({
            name: `Nail Upgrades`,
            value: nailUpgrades,
            status: nailUpgrades >= 4 ? 'completed' : (nailUpgrades > 0 ? 'partial' : 'missing'),
            description: `${nailUpgrades}/4 nail upgrades${nailUpgrades < 4 ? ` (${4 - nailUpgrades} missing)` : ''}`
        });
        ...
        items.push({
            ...
            status: toolKitUpgrades >= 4 ? 'completed' : (toolKitUpgrades > 0 ? 'partial' : 'missing'),
            description: `${toolKitUpgrades}/4 tool kit upgrades${toolKitUpgrades < 4 ? ` (${4 - toolKitUpgrades} missing)` : ''}`
        });
        ...
        items.push({
            ...
            status: toolPouchUpgrades >= 4 ? 'completed' : (toolPouchUpgrades > 0 ? 'partial' : 'missing'),
            description: `${toolPouchUpgrades}/4 tool pouch upgrades${toolPouchUpgrades < 4 ? ` (${4 - toolPouchUpgrades} missing)` : ''}`
        });
        ...
        abilities.forEach(ability => {
            if (ability.has) {
                completion += 1;
                items.push({ name: ability.name, value: 1, status: 'completed', description: 'Unlocked' });
            } else {
                items.push({ name: ability.name, value: 0, status: 'missing', description: 'Not unlocked' });
            }
        });
        ...
        items.push({
            ...
            value: healthValue,
            status: (playerData.maxHealthBase || 5) >= 10 ? 'completed' : ((playerData.maxHealthBase || 5) > 5 ? 'partial' : 'missing'),
            description: `${playerData.maxHealthBase || 5}/10 max health (${healthValue} count towards completion)${(playerData.maxHealthBase || 5) < 10 ? ` (${10 - (playerData.maxHealthBase || 5)} missing)` : ''}`
        });
        ...
        items.push({
            name: `Silk Upgrades`,
            value: silkValue,
            status: (playerData.silkMax || 9) >= 18 ? 'completed' : ((playerData.silkMax || 9) > 9 ? 'partial' : 'missing'),
            description: `${playerData.silkMax || 9}/18 max silk (${silkValue} count towards completion)${(playerData.silkMax || 9) < 18 ? ` (${18 - (playerData.silkMax || 9)} missing)` : ''}`
        });
        ...
        specialItems.forEach(item => {
            if (item.has) {
                completion += 1;
                items.push({ name: item.name, value: 1, status: 'completed', description: 'Found' });
            } else {
                items.push({ name: item.name, value: 0, status: 'missing', description: 'Not found' });
            }
        });
    ...
    getUnlockedToolsCount() {
        if (!this.saveData || !this.saveData.playerData || !this.saveData.playerData.Tools || !this.saveData.playerData.Tools.savedData) {
            return 0;
        }
    ...
    getUnlockedCrestsCount() {
        if (!this.saveData || !this.saveData.playerData || !this.saveData.playerData.ToolEquips || !this.saveData.playerData.ToolEquips.savedData) {
            return 0;
        }
        const crestsData = this.saveData.playerData.ToolEquips.savedData;
        let count = 0;
        for (const crest of crestsData) {
            if (crest.Data && crest.Data.IsUnlocked && crest.Name.toLowerCase() !== "cursed") {
                count++;
            }
        }
        return count;
    ...
    hasWhiteFlower() {
        if (!this.saveData || !this.saveData.playerData || !this.saveData.playerData.Collectables || !this.saveData.playerData.Collectables.savedData) {
            return false;
        }
        const collectablesData = this.saveData.playerData.Collectables.savedData;
        for (const collectable of collectablesData) {
            if (collectable.Name === "White Flower" && collectable.Data && collectable.Data.Amount > 0) {
                return true;
            }
        }
        return false;
    ...
    getMissingCrests() {
        const knownCrests = ['Hunter', 'Reaper', 'Wanderer', 'Warrior', 'Hunter_v2', 'Toolmaster', 'Witch', 'Spell'];
        if (!this.saveData || !this.saveData.playerData || !this.saveData.playerData.ToolEquips || !this.saveData.playerData.ToolEquips.savedData) {
            return knownCrests;
        }
...
function checkCompletion() {
    try {
        ...
        const completionFromSaveRaw = findValueByKeyInsensitive(tracker.saveData, 'completionpercentage');
        const completionPercent = Math.max(0, Math.min(100, Number.parseFloat(completionFromSaveRaw ?? '0') || 0));
        ...
        const categories = {
            'Tools & Equipment': completionBreakdown.items.filter(item =>
                item.name.includes('Tools') || item.name.includes('Crests') ||
                item.name.includes('Nail') || item.name.includes('Kit') ||
                item.name.includes('Pouch')
            ),
            'Abilities': completionBreakdown.items.filter(item =>
                item.name.includes('Dash') || item.name.includes('Jump') ||
                item.name.includes('Needolin') || item.name.includes('Charge')
            ),
            'Upgrades': completionBreakdown.items.filter(item =>
                item.name.includes('Health') || item.name.includes('Silk')
            ),
            'Special Items': completionBreakdown.items.filter(item =>
                item.name.includes('Bound') || item.name.includes('White')
            )
        };
        ...
        for (const [categoryName, items] of Object.entries(categories)) {
            if (items.length > 0) {
                ...
                `;
                items.forEach(item => {
                    const statusClass = item.status === 'completed' ? 'status-completed' : 'status-missing';
                    html += `
        ...
        if (fleaCounts.total > 0) {
            const fleaPercent = Math.round((fleaCounts.found / fleaCounts.total) * 100);
            ...
            html += `
                <div class="completion-category">
                    ...
                    <div class="progress-text">${fleaCounts.found}/${fleaCounts.total} (${fleaPercent}%)</div>
                    <div class="completion-item">
                        <div class="item-name">Fleas Rescued</div>
                        <div class="item-status ${fleaCounts.found === fleaCounts.total ? 'status-completed' : (fleaCounts.found > 0 ? 'status-partial' : 'status-missing')}">
                    ...
                    <div class="completion-item">
                        ...
                        <div class="item-status ${fleaCounts.giantTamed ? 'status-completed' : 'status-missing'}">
        ...
        if (missingCrests.length > 0 || missingTools.length > 0) {
            ...
            if (missingCrests.length > 0) {
                html += `
                    <div style="margin-bottom: 15px;">
                        <strong>Missing Crests (${missingCrests.length}):</strong><br>
                        <span style="color: #666; font-size: 0.9em;">${missingCrests.join(', ')}</span>
                    </div>
                `;
            }
            if (missingTools.length > 0) {
                html += `
                    <div style="margin-bottom: 15px;">
                        <strong>Missing Tools (${missingTools.length}):</strong><br>
                        <span style="color: #666; font-size: 0.9em;">${missingTools.slice(0, 10).join(', ')}${missingTools.length > 10 ? ` ... and ${missingTools.length - 10} more` : ''}</span>
                    </div>
                `;
            }
            html += `</div>`;
        ...
        try { completionResults.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); } catch (_) {}
    } catch (error) {
...
function showStats() {
    try {
        ...
        const playTimeHMS = formatHMS(typeof playTimeRaw === 'string' ? parseFloat(playTimeRaw) : playTimeRaw);
        ...
        html += `<div class="completion-item"><div class="item-name">Health</div><div class="item-status ${health != null ? 'status-completed' : 'status-missing'}">${health ?? 'N/A'}</div></div>`;
        html += `<div class="completion-item"><div class="item-name">Silk</div><div class="item-status ${silk != null ? 'status-completed' : 'status-missing'}">${silk ?? 'N/A'}</div></div>`;
        html += `<div class="completion-item"><div class="item-name">Rosaries</div><div class="item-status ${rosaries != null ? 'status-completed' : 'status-missing'}">${rosaries ?? 'N/A'}</div></div>`;
        html += `<div class="completion-item"><div class="item-name">Shell Shards</div><div class="item-status ${shellShards != null ? 'status-completed' : 'status-missing'}">${shellShards ?? 'N/A'}</div></div>`;
        html += '</div>';
        statsResults.innerHTML = html;
        try { statsResults.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); } catch (_) {}
    } catch (e) {
        statsResults.innerHTML = `<div class="error">❌ Error reading stats: ${e.message}</div>`;
    }
}
...
function showFleaTracking() {
    try {
        ...
        let html = `
            <div class="completion-category">
                ...
                <div class="completion-item">
                    ...
                    <div class="item-status ${unfreedCount === 0 ? 'status-completed' : 'status-missing'}">${unfreedCount}</div>
                </div>
        `;
        ...
        try { fleaResults.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); } catch (_) {}
    } catch (error) {
...
function getFleaCounts(saveData) {
    const result = { total: 0, found: 0, giantFoundPresent: false, giantFound: false };
